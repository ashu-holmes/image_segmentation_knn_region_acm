...
curvature = get_curvature(phi,idx); % force from curvature penalty
 ...
 dphidt = F./max(abs(F)) + alpha*curvature; % gradient descent to minimize 
energy
...
%m(2:size(I,1)-1,2:size(I,2)-1)=; 
m((size(I,1)*1)/8:(size(I,1)*7)/8,(size(I,2)*1)/8:(size(I,2)*7)/8) = 1; 

I = imresize(I,2); %-- make image smaller 
m = imresize(m,2); % for fast computation 

subplot(2,2,1); imshow(I); title('Input Image'); 
subplot(2,2,2); imshow(m); title('Initialization'); 
subplot(2,2,3); title('Segmentation'); 

seg = region_seg(I, m, 700); %-- Run segmentation 
imshow(I); 

subplot(2,2,4); 

imshow(seg); title('Global Region-Based Segmentation'); 
seg = imresize(seg,0.5); %-- make image smaller 
imwrite(seg,'output5.png'); 
